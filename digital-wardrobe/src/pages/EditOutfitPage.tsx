...
const EditOutfitPage: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            setLoading(true);
            try {
                const [outfit, allItems] = await Promise.all([
                    outfitService.fetchOutfit(id),
                ...
                if (outfit) {
                    ...
                    setSeason(outfit.season || 'spring');
                    setSelectedItemIds(outfit.items.map(i => i.id));
    ...
    const handleDelete = async () => {
        if (!id) { return; }
        if (!window.confirm('确定要删除这个搭配计划吗？')) { return; }
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark min-h-screen pb-20">
            ...
            <div className="flex items-center bg-white dark:bg-surface-dark p-4 justify-between sticky top-0 z-30 border-b border-gray-100 dark:border-gray-800">
                ...
                <h2 className="text-lg font-bold">编辑搭配</h2>
                <button
                    type="button"
                    onClick={() => {
                        if (step === 'details') {
                ...
                >
                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : (step === 'details' ? '下一步' : '更新')}
                </button>
            </div>
            ...
            {step === 'details' && (
                <div className="p-4 space-y-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {['日常', '工作', '约会', '运动', '度假'].map(occ => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${occasion === occ ? 'bg-primary text-text-main' : 'bg-white dark:bg-surface-dark text-text-secondary hover:bg-gray-50'}`}
                                >
                                    {occ}
                                </button>
                            ))}
                    ...
                    <div className="space-y-2">
                        <span className="text-sm font-bold text-text-secondary">状态</span>
                        <div className="flex gap-2">
                            {(['planned', 'completed'] as const).map(s => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setStatus(s)}
                                    className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${status === s ? 'bg-text-main text-white dark:bg-primary dark:text-text-main' : 'bg-white dark:bg-surface-dark text-text-secondary'}`}
                                >
                                    {s === 'planned' ? '计划中' : '已穿着'}
                                </button>
                            ))}
                    ...
                    <div className="pt-8">
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 py-4 rounded-xl text-red-500 font-bold bg-red-50 dark:bg-red-500/10 hover:bg-red-100 transition-colors"
                        >
                        ...
                        {wardrobeItems.map(item => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`relative aspect-square rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${isSelected ? 'border-primary scale-95' : 'border-transparent'}`}
                                >
                                    <img src={item.imageUrl || 'https://via.placeholder.com/150'} alt={item.name} className="w-full h-full object-cover" />
                                    {isSelected && (
                                        <div className="absolute top-2 right-2 bg-primary text-text-main rounded-full p-1 shadow-sm">
